...
TOOL_NAME = "five9_remove_skills_from_campaign"
SOAP_METHOD = "removeSkillsFromCampaign"


def five9_remove_skills_from_campaign(
    campaign_name: str,
    skills: list[str] | str,
    mode: str = "dry_run",
    approved: bool = False,
    ...
    skill_values = require_list(skills, "skills")
    params = clean_params(campaign_name=campaign_name, skills=skill_values)
    if mode == "dry_run":
        return build_planned_write_result(TOOL_NAME, params, SOAP_METHOD)

    require_live_write(mode, approved, client)
    data = client.remove_skills_from_campaign(**params)
